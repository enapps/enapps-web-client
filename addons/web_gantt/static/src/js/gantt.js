...
openerp.web_gantt.GanttView = openerp.web.View.extend({
    ...
    do_search: function (domains, contexts, group_bys) {
        ...
        _.each(self.fields_view.arch.children, function(child) {
            fields.push(child.attrs.name);
        });
    ...
    on_data_loaded_2: function(tasks, group_bys) {
        ...
        var generate_task_info = function(task, plevel) {
            ...
            if (task.__is_group) {
                ...
                if (level == 0) {
                    var assigned_to = tasks[level].assigned_to;
                    var group = new GanttProjectInfo(_.uniqueId("gantt_project_"), group_name, assigned_to, task_start);
                ...
                // Added User Assigned to property
                var assigned_to = task.assigned_to;
                var task_info = new GanttTaskInfo(id, task_name, task.assigned_to, task_start, ((duration / 24) * 8), 100);
