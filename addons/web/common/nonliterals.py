...
_ALLOWED_KEYS = frozenset(['__ref', "__id", '__domains', '__debug',
                           ...
                           'active_model', 'active_id', 'active_ids', 'context_from_parent_node',
                           'dataset_ids_when_form_open'])
...
class Domain(BaseDomain):
    ...
    def evaluate(self, context=None):
        ...
        try:
            final_context = {}
            for k, v in SuperDict(ctx).items():
                if v and isinstance(v, list) and isinstance(v[0], list) and len(v[0]) == 3:
                    final_context[k] = v[0][2]
                else:
                    final_context[k] = v
            if final_context.get('parent'):
                final_context['parent'] = AttributeDict(final_context.get('parent'))
            return eval(self.get_domain_string(), final_context)
...
class Context(BaseContext):
    ...
    def evaluate(self, context=None):
        ...
        evaluated result.
        """
        ctx = non_literal_decoder(context)
        if self.own:
            ctx.update(self.own)
        try:
            result = eval(self.get_context_string(), SuperDict(ctx))
            if self.own:
                result.update(self.own)
            return result
        except NameError as e:
...
class CompoundContext(BaseContext):
    ...
    def evaluate(self, context=None):
        ...
        final_context.update(eval_context)
