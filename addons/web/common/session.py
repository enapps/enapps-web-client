...
class OpenERPSession(object):
    ...
    def __init__(self):
        ...
        self.contexts_store = {}
        self.domains_store = {}
        self.domain_level_db_name = int(openerp.tools.config.options.get('domain_level_db_name', 0))
        self.domain_level_user_name = int(openerp.tools.config.options.get('domain_level_user_name', 0))
        self.groups = []
    ...
    def build_connection(self):
        if self._db in config.options.get('do_not_allow_db_list').split(','):
            raise Exception('Error', "database %s is not allowed for this instace of OpenERP" % self._db)
    ...
    def authenticate(self, db, login, password, env):
        ...
        if db in config.options.get('do_not_allow_db_list').split(','):
            return False
    ...
    def get_groups_names(self):
        if self._db:
            self.groups = self.proxy('common').get_groups_names(self._db, self._uid)
