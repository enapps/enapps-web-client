...
def admin_required(f):
    ...
    def wrapper(*args, **kwds):
        controller, request, options = args
        admin_session = request.cookies.get('admin_session')
        if not (
            admin_session and
            SecureCookie.unserialize(admin_session, secret_key=SECRET_KEY).get('signature') == signature()
        ):
            return werkzeug.utils.redirect(get_admin_url() + '/login')
        return f(*args, **kwds)
    return wrapper

def signature():
    return config.options.get('admin_user') + config.options.get('admin_passwd')

def get_admin_url():
    return "/%s" % config.options.get('admin_url', 'admin')
...
class WebClient(openerpweb.Controller):
    ...
    def manifest_list(self, req, mods, extension):
        if not req.debug:
            ...
            if mods is not None:
                path += '?mods=' + mods
            if extension=='js' and not mods:
                # throw compressed JS for main webclient code
                if os.path.exists('static/build/web_minified.js'):
                    path='../../web/static/build/web_minified.js'
            return [path]
    ...
    def js(self, req, mods=None):
        ...
        js_list = None
        if os.path.exists('compressed_files.txt'):
            with open('compressed_files.txt','r') as f:
                js_list=f.read()
                if js_list:
                    js_list=eval(js_list)
        if not js_list or js_list!=files or not os.path.exists('min_js.js') \
            or os.path.getmtime('min_js.js') <= max(map(lambda x: os.path.getmtime(x), files)):
            try:
                logging.info('Compressing JavaScript')
                with open('pre_js.js', 'w') as f:
                    f.write(content)
                subprocess.call(['java','-jar', 'compiler.jar','--language_in=ECMASCRIPT5', '--js','pre_js.js','--js_output_file','min_js.js'])
                with open('min_js.js', 'r') as f:
                    content = f.read()
                last_modified = datetime.datetime.fromtimestamp(os.path.getmtime('min_js.js'))
                os.remove('pre_js.js')
                with open('compressed_files.txt','w') as f:
                    simplejson.dump(files,f)
            except:
                logging.warn('Closure Compiler Error')
        else:
            with open('min_js.js', 'r') as f:
                content = f.read()
        return self.make_conditional(
...
class Database(openerpweb.Controller):
    ...
    def get_list(self, req):
        ...
        dbs = proxy.list()
        host_name = req.httprequest.environ['HTTP_HOST'].split(':')[0]
        domain_name = host_name.split('.')[0]
        result_filter = req.config.dbfilter.replace('%h', host_name).replace('%d', domain_name)
        dbs = [i for i in dbs if re.match(result_filter, i)]
        do_not_allow_db_list = map(str.strip, config.options.get('do_not_allow_db_list').split(','))
        dbs = list(set(dbs) - set(do_not_allow_db_list))
        dbs.sort()
        return {"db_list": dbs}
    ...
    @admin_required
    @openerpweb.jsonrequest
    def clone(self, req, fields):
        params = dict(map(operator.itemgetter('name', 'value'), fields))
        clone_attrs = (
            params['clone_pwd'],
            params['clone_db'],
            params['clone_db_name']
        )
        remove = ['clone_pwd', 'clone_db', 'clone_db_name', 'token']
        for item in remove: del params[item]
        return req.session.proxy("db").clone(*clone_attrs, **params)

    @admin_required
    @openerpweb.jsonrequest
    def migrate(self, req, fields):
        try:
            migrate_pwd = [dct.get('value') for dct in fields if dct.get('name') == 'migrate_pwd'][0]
            migrate_dbs = [dct.get('value') for dct in fields if dct.get('name') == 'migrate_db']
            if not migrate_dbs:
                return {'error': 'Please, select database at first!', 'title': 'Migrate Database'}
            return req.session.proxy("db").migrate_databases(migrate_pwd, migrate_dbs)
        except xmlrpclib.Fault, e:
            if e.faultCode and e.faultCode.split(':')[0] == 'AccessDenied':
                return {'error': 'Master Pasword is not correct!', 'title': 'Migrate Database'}
        return {'error': 'Could not migrate database! \nMaybe error present in:\n %s' % (e.faultCode), 'title': 'Migrate Database'}

    @admin_required
    @openerpweb.httprequest
    def backup(self, req, backup_db, backup_pwd, token):
        token = token or '1'
        db_dump = req.session.proxy("db").dump(backup_pwd, backup_db)
    ...
    @admin_required
    @openerpweb.httprequest
    def restore(self, req, db_file, restore_pwd, new_db, replace_all_emails=False, new_email='',
                replace_all_passwd=False, new_passwd='',
                disable_all_crons=False):
        try:
            req.session.proxy("db").restore(restore_pwd, new_db, db_file,
                                            replace_all_emails=replace_all_emails and True or False, new_email=new_email,
                                            replace_all_passwd=replace_all_passwd and True or False, new_passwd=new_passwd,
                                            disable_all_crons=disable_all_crons and True or False, )
            return ''
        except xmlrpclib.Fault, e:
            if e.faultCode and e.faultCode.split(':')[0] == 'AccessDenied':
...
class Session(openerpweb.Controller):
    ...
    def session_info(self, req):
        domain_level_db_name = int(config.options.get('domain_level_db_name', 0))
        domain_level_user_name = int(config.options.get('domain_level_user_name', 0))
        domain_list = map(str.strip, config.options.get('domain_list').split(','))
        host, port = req.httprequest.host.split(':')
        db = None
        login = None
        for base_domain in domain_list:
            if host.endswith('.' + base_domain):
                top_level_domain_list = host.rpartition('.' + base_domain)[0].split('.')
                top_level_domain_list.reverse()
                if domain_level_db_name and len(top_level_domain_list) >= domain_level_db_name:
                    db = top_level_domain_list[domain_level_db_name - 1]
                if domain_level_user_name and len(top_level_domain_list) >= domain_level_user_name:
                    login = top_level_domain_list[domain_level_user_name - 1]
                break
        req.session.ensure_valid()
        req.session.get_groups_names()
        return {
            "session_id": req.session_id,
            "uid": req.session._uid,
            "context": req.session.get_context() if req.session._uid else {},
            "db": db or req.session._db,
            "domain_level_db_name": domain_level_db_name or None,
            "domain_level_user_name": domain_level_user_name or None,
            "login": login or req.session._login,
...
class Menu(openerpweb.Controller):
    ...
    def do_load(self, req):
        ...
        menu_roots = Menus.read(self.do_get_user_roots(req), ['name', 'sequence', 'parent_id', 'icon', 'icon_color', ], context)
        ...
        menu_items = Menus.read(menu_ids, ['name', 'sequence', 'parent_id', 'icon', 'icon_color', ], context)
...
class DataSet(openerpweb.Controller):
    ...
    def default_get(self, req, model, fields):
        context = req.session.eval_context(req.context)
        Model = req.session.model(model)
        return Model.default_get(fields, context)
...
class View(openerpweb.Controller):
    ...
    def parse_domains_and_contexts(self, elem, session):
        ...
        """
        for el in ['domain', 'filter_domain', 'ns_filter_domain']:
...
class ListView(View):
    ...
    def run_object_action(self, req, action_id, res_ids):
        ir_action_object_model = req.session.model('ir.actions.object')
        result = ir_action_object_model.run_object_action(
            action_id,
            res_ids, )
        return result
...
class Binary(openerpweb.Controller):
    ...
    def image(self, req, model, id, field, **kw):
        ...
        try:
            ...
            if field == 'avatar' and ('avatar' in kw) and kw.get('avatar')=='1':
                ...
                image_data = self.default_image(req,'logo_web')
            else:
    ...
    def saveas(self, req, model, field, id=None, filename_field=None, **kw):
        ...
        if not filecontent:
            ...
            contentTypeModel = req.session.model('ir.mime')
            return req.make_response(filecontent,
                [('Content-Type', contentTypeModel.get_mime(filename)),
                ('Content-Disposition', 'filename="%s"' % (filename)),])

    @openerpweb.httprequest
    def open_file(self, req, data):
        ...
        if not filecontent:
            ...
            if filename_field:
                filename = res.get(filename_field, '') or filename
            return req.make_response(filecontent,
                headers=[('Content-Type', contentTypeModel.get_mime(filename)), ],)

    @openerpweb.httprequest
    def saveas_ajax(self, req, data, token=None):
        jdata = simplejson.loads(data)
        model = jdata['model']
        field = jdata['field']
        id = jdata.get('id', None)
...
class Action(openerpweb.Controller):
    ...
    @openerpweb.jsonrequest
    def load(self, req, action_id, model=None, do_not_eval=False):
        Actions = req.session.model('ir.actions.actions')
        if isinstance(action_id, (str, unicode)):
            ModelData = req.session.model('ir.model.data')
            action_id = ModelData.get_object(model, action_id).id
        value = False
        context = req.session.eval_context(req.context)
...
class DBAdmin(WebClient):
    _cp_path = get_admin_url()

    init_string = '''
    db = new s.web.Database();
    db.appendTo($('.openerp'));
    '''
    html_template = """<!DOCTYPE html>
    <html style="height: 100%%">
        <head>
            ...
            <script type="text/javascript">
                $(function() {
                    s = new openerp.init(%(modules)s);
                    $.get('/web/webclient/qweb', function (xml) {
                        s.web.qweb.add_template(xml);
                        %(init)s
                    });
                });
            </script>
    ...
    login_templete = """<html style="height: 100%"><head>
        ...
        <body class="openerp" id="oe" style="">
            <div class="login">
                <div id="login-menu-bar">
                    <div class="login-menu-header">
                        <h2 class="menu-title header-title">
                            <img class="header-logo" src="/web/static/src/img/logo.png">
                ...
                <div class="login-pane pane">
                    <form action="login" method="post">
                        <ul>
                            <li style="false"><input autofocus="autofocus" name="login" placeholder="Login" type="text" value=""></li>
                            <li><input name="password" placeholder="Password" type="password" value=""></li>
...
"""
    ...
    def index(self, *args, **kwargs):
        return super(DBAdmin, self).home(*args, **kwargs)

    @openerpweb.httprequest
    def login(self, req, login=None, password=None, **kwargs):
        if config.options.get('admin_user') == login and config.options.get('admin_passwd') == password:
            admin_session = SecureCookie({
                'signature': signature(),
            }, secret_key=SECRET_KEY).serialize()
            result = werkzeug.utils.redirect(self._cp_path)
            result.set_cookie('admin_session', admin_session)
            return result
        else:
