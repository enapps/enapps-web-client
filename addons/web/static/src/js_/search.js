...
openerp.web.SearchView = openerp.web.OldWidget.extend(/** @lends openerp.web.SearchView# */{
    ...
    searchview_widget: function() {
        if (!(!this.widget_parent.$element.is('.ui-dialog-content') && $('.main_table').attr('version')=="v7_t"))
            return;
        var self = this,
            $first_input = this.$element.find('input[type="text"]:visible:first'),
            $form = self.$element.find('.oe_forms'),
            _right_position = self.$element.find('.search_top_field').width(),
            searchview_css_show = {
                'right':(_right_position-150).toString() + "px",
            };
        var unfold_search_form = (function(){
            var _search_top_pos = -35;
                $form.removeClass('folded');
                $first_input.removeClass('at_search_widget').css('position','initial');
                self.$element.find('.oe_searchview_facet').css('display','none');
                self.$element.find('.search_top_field').addClass('expanded').css('top',_search_top_pos);
                // $control_buttons.addClass('top_searchview_widget').css('top',_search_top_pos+2);
                // $control_buttons[0].style.right = (_right_position-50).toString()+"px";
                var $reset_search = $form.find('[type="reset"]');
                $reset_search.css('right','40px');
                $form.find('.oe_search-view-filters-management').addClass('search_top_field').css({
                    'top':_search_top_pos,
                    'margin-top': '2px',
                    'right': '135px'
                });
                $form.find('oe_searchview_unfold_drawer').css({'-webkit-transform': 'rotate(-180deg)','padding-left': '15px'});
                window.localStorage.searchview_folded = false;
        }),
            fold_search_form = (function(){
                self.$element.find('.search_top_field').css('top','');
                self.$element.find('.oe_searchview_facet').css('display','');
                self.$element.find('.search_top_field').removeClass('expanded')
                $form.addClass('folded');
                $first_input.addClass('at_search_widget').css('position','absolute');
                window.localStorage.searchview_folded = true;
            });
        if (!!window.localStorage.searchview_folded) {
            if (window.localStorage.searchview_folded == 'true') {
                $form.addClass('folded');
                fold_search_form();
            } else {
                $form.removeClass('folded');
        ...
        var $pseudo_label = $first_input.closest('.oe_searchview_field').find('label[for="'+$first_input.attr('id')+'"]').clone();
        this.$element.find('.search_top_field .oe_facet_value').text(
            $pseudo_label.children().remove().end().text().trim()
            );
        $form.find('.oe_searchview_facet').css(searchview_css_show);
        var first_input_css_show = {
                'right':(_right_position-self.$element.find('.oe_searchview_facet').outerWidth()-160).toString() + "px"
            };
        $first_input.css(first_input_css_show);
        this.$element.find('.oe_searchview_unfold_drawer').unbind('click').bind('click',function(){
            if (self.$element.find('.oe_forms').css('visibility')=='hidden') {
                unfold_search_form();
            } else {
                fold_search_form();
            }
        })
        this.$element.find('div.oe_search_top_clear').unbind('click').click(function() {self.$element.find('form').trigger('reset');});
        this.$element.find('button.oe_searchview_search').unbind('click').click(function(){self.do_search()});
    },
    ...
    do_clear: function (reload_view) {
        ...
        return $.async_when.apply(
            null, _(this.inputs).invoke('clear')).then(
...
openerp.web.search.FloatField = openerp.web.search.NumberField.extend(/** @lends openerp.web.search.FloatField# */{
    ...
    start: function() {
        ...
        this.$element2 = $('<input>',{
            ...
            'id': _.uniqueId(
                ['search_input'].concat(
                    _.compact([self.attrs.type,self.attrs.name]),
                    ['']).join('_')),
            'size':'15',
        }).removeAttr('style');
        this.$element.after(' - ',this.$element2);
    ...
    get_value2: function() {
        if (!this.$element2.val()) {
            return null;
        }
        var val = this.parse(this.$element2.val()),
          check = Number(this.$element2.val());
        if (isNaN(val) || val !== check) {
            this.$element2.addClass('error');
            throw new openerp.web.search.Invalid(
                this.attrs.name, this.$element2.val(), this.error_message);
        }
        this.$element2.removeClass('error');
        return val;
    },
    get_domain: function() {
        var val = this.get_value();
...
openerp.web.search.ManyToOneField = openerp.web.search.CharField.extend({
    init: function (view_section, field, view) {
        ...
        var self = this;
        this.got_name = $.Deferred().done(function () {
    ...
    setup_autocomplete: function () {
        var self = this, ignore_blur = false;
        this.$element.bind({
            keydown: function(event) {
                if (event.which == 9) {
                    ...
                    {
                        self.$element.val(self.last_search[0][1]);
                        self.id = self.last_search[0][0];
                        self.name = self.last_search[0][1];
                    }
                }
            },
        });
        this.$element.autocomplete({
            source: function (req, resp) {
                ...
                //ns_filter_domain: name search filter domain is passed through name_search
                if (self.attrs.ns_filter_domain) {
                    domain = $.merge(self.attrs.ns_filter_domain, self.attrs.domain);
                }
                else {
                ...
                self.dataset.name_search(
                    req.term, domain, 'ilike', 8, function (data) {
...
openerp.web.search.ExtendedSearchProposition = openerp.web.OldWidget.extend(/** @lends openerp.web.search.ExtendedSearchProposition# */{
    ...
    start: function () {
        ...
        this.$element.find(".searchview_extended_prop_op").change(function() {
            var $value = self.$element.find('.searchview_extended_prop_value');
            switch($(this).val()) {
            case '∃':
            case '∄':
                $value.hide();
