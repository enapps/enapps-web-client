...
openerp.web.BufferedDataSet = openerp.web.DataSetStatic.extend({
    ...
    create: function(data, callback, error_callback) {
        ...
        var prom = $.Deferred().done(callback);
    ...
    write: function (id, data, options, callback) {
        ...
        if (dirty)
            this.on_change();
        var to_return = $.Deferred().done(callback);
    ...
    sort_cache: function() {
        var self = this, temp_cache = [];
        for (i in self.ids) {
            self.cache.filter(function(el){
                if (el.id===self.ids[i] && !_.isEmpty(el.values)) {
                    temp_cache.push({'id':self.ids[i],'values':el.values,}); return el
                }
            })
        }
...
openerp.web.ProxyDataSet = openerp.web.DataSetSearch.extend({
    ...
    default_get: function(fields, options) {
        var self = this;
        return this._super(fields, options).done(function(result){ return self.on_default_get(result); });
    },
