...
openerp.web.format_cell = function (row_data, column, options) {
    ...
    if (column.tag === 'button') {
        return _.template('<button type="button" title="<%-title%>" <%=additional_attributes%> >' +
            (column.icon && column.icon.match('^icon') !== null ? '<span class="<%-icon%>" style="color: <%-icon_color%>"/>' : '<img src="<%-prefix%>/web/static/src/img/icons/<%-icon%>.png" alt="<%-alt%>"/>') +
            '</button>', {
                title: column.string || '',
                additional_attributes: isNaN(row_data.id.value) && openerp.web.BufferedDataSet.virtual_id_regex.test(row_data.id.value) ?
                    'class="oe-listview-button"' : '',
                prefix: openerp.connection.prefix,
                icon: column.icon,
                icon_color: column.icon_color && column.icon_color || '',
                alt: column.string || ''
            });
    }
    ...
    case "boolean":
        return _.str.sprintf('<input type="checkbox" %s '+(column.readonly=="1"?'disabled':'')+'/>',
