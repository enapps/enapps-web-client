...
openerp.web.Dialog = openerp.web.OldWidget.extend(/** @lends openerp.web.Dialog# */{
    ...
    init: function (parent, options, content) {
        ...
        this.dialog_options = {
            ...
            position: [false, 50],
            buttons: {},
            beforeClose: function () { try { self.widget_parent.dialog_stop && self.widget_parent.dialog_stop(); } catch(err) { self.on_close(); } },
        ...
        if (this.dialog_options.autoOpen) {
            ...
            this.$element.dialog(this.get_options()).dialogExtend({
                "maximize": true,
            });
        }
    },
    ...
    open: function(options) {
        ...
        this.$element.dialog(o).dialogExtend({
                "maximize": true,
            }).dialog('open');
    ...
    close: function() {
        try { this.$element.dialog('close'); } catch(error) {console.warn('Could not call method Close of Dialog',error);}
    },
    on_close: function() {
        if (this.dialog_options.destroy_on_close && !!this.$element.data('ui-dialog')) {
...
openerp.web.CrashManager = openerp.web.CallbackEnabled.extend({
    ...
    on_managed_error: function(error) {
        $('<div>' + QWeb.render('CrashManagerWarning', {error: error}) + '</div>').dialog({
            title: "Enapps " + _.str.capitalize(error.type),
            buttons: [
    ...
    on_traceback: function(error) {
        ...
        if (openerp.connection.openerp_entreprise) {
            buttons[_t("Send Enapps Enterprise Report")] = function() {
                var $this = $(this);
                var issuename = $('#issuename').val();
                var explanation = $('#explanation').val();
                var remark = $('#remark').val();
                // Call the send method from server to send mail with details
                new openerp.web.DataSet(self, 'publisher_warranty.contract').call_and_eval('send', [error.data,explanation,remark,issuename]).done(function(result){
                    if (result === false) {
...
openerp.web.Database = openerp.web.OldWidget.extend(/** @lends openerp.web.Database# */{
    ...
    start: function() {
        ...
        var self = this;
        var fetch_db = this.rpc("/web/database/get_list", {}).done(
            function(result) { self.db_list = result.db_list; }).fail(
        ...
        $.when(fetch_db, fetch_langs).then(function () {self.do_list();});

        this.$element.find('#db-list').click(this.do_list);
        ...
        this.$element.find('#db-clone').click(this.do_clone);
        this.$element.find('#db-migrate').click(this.do_migrate);
        ...
        this.$element.delegate('li', 'click', function(e) {
            self.$element.find('li').removeClass('selected');
    ...
    stop: function () {
        ...
        this.$element
            .find('#db-create, #db-drop, #db-backup, #db-clone, #db-restore, #db-change-password, #back-to-login')
                .unbind('click')
            .end()
            .empty();
        this._super();
    ...
    do_list: function() {
        ...
        self.$option_id.html(QWeb.render("Database.ListDB", self));
    },
    do_create: function() {
        var self = this;
    ...
    do_clone: function() {
        var self = this;
        self.$option_id
            .html(QWeb.render("CloneDB", {'widget': self}))
            .find("form[name=clone_db_form]").validate({
            submitHandler: function (form) {
                self.blockUI();
                localStorage.setItem('new_email', $(form).find('input[name="new_email"]').val());
                localStorage.setItem('new_passwd', $(form).find('input[name="new_passwd"]').val());
                var $form = $(form),
                    fields = $form.serializeArray(),
                    $db_list = $form.find('[name=drop_db]'),
                    db = $db_list.val();
                self.rpc("/web/database/clone", {'fields': fields}, function(result) {
                    if (result.error) {
                        self.display_error(result);
                        return;
                    }
                    self.unblockUI();
                    self.do_notify("Cloning database", "The database '" + db + "' has been cloned");
                });
            }
        });
    },
    do_migrate: function() {
        var self = this;
        self.$option_id
            .html(QWeb.render("MigrateDB", self))
            .find("form[name=migrate_db_form]").validate({
            submitHandler: function (form) {
                var $form = $(form),
                    fields = $form.serializeArray(),
                    $db_list = $form.find('[name=drop_db]'),
                    db = $db_list.val();
                self.blockUI();
                self.rpc("/web/database/migrate", {'fields': fields}, function(result) {
                    if (result.error) {
                        self.display_error(result);
                        self.unblockUI();
                        return;
                    ...
                    self.unblockUI();
                    self.do_notify("Migration database", "The database '" + db + "' has been migrated");
                });
            }
        });
    },
    do_restore: function() {
        var self = this;
       	self.$option_id.html(QWeb.render("RestoreDB", {'widget': self}));
...
openerp.web.Login =  openerp.web.OldWidget.extend(/** @lends openerp.web.Login# */{
    ...
    init: function(parent) {
        ...
        this.selected_db = this.session.db;
        this.selected_login = this.session.username;

        if (!this.session.domain_level_user_name && this.has_local_storage && this.remember_credentials) {
            this.selected_db = this.selected_db || localStorage.getItem('last_db_login_success');
    ...
    set_db_list: function (list) {
        ...
        if (this.selected_db && list.indexOf(this.selected_db) == -1) {
            localStorage.removeItem('last_db_login_success');
            self.$element.find('form').replaceWith('<h2>Database "' + self.selected_db + '" not registered on server</h2>');
        }
    },
    ...
    do_login: function (db, login, password) {
        ...
        this.session.session_authenticate(db, login, password).then(function() {
            !!self.$element && self.$element.removeClass("login_invalid");
...
openerp.web.Header =  openerp.web.OldWidget.extend(/** @lends openerp.web.Header# */{
    ...
    do_update: function () {
        ...
        var fct = function() {
            ...
            return func(self.session.uid, self.fields).then(function(res) {
                self.session = _.extend(self.session, res);
    ...
    shortcut_load :function(){
        var self = this,
            sc = openerp.webclient.session.shortcuts,
...
openerp.web.Menu =  openerp.web.OldWidget.extend(/** @lends openerp.web.Menu# */{
    ...
    init: function(parent, element_id, secondary_menu_id) {
        ...
        this.slider_header_down = $('#sliderhdrdown');
        this.slider_header_up = $('#sliderhdrup');
        this.menu = false;
        this.folded = true;
        if (this.slider_header_down.length != 0 && this.slider_header_up.length != 0) {
            this.sliderhdr = false;
            if (window.localStorage) {
                if (localStorage.getItem('oe_menu_folded') === 'false') {
                    this.folded = false;
                };
                this.sliderhdr = localStorage.getItem('header_menu_folded') === 'true';
            }
        } else {
            this.sliderhdr = this.slider_header_down = this.slider_header_up = null;
        }
        this.float_timeout = 700;
    },
    start: function() {
        if (this.sliderhdr !== null) {
            this.sliderhdr ? this.header_hide() : this.header_show();
        }
    ...
    header_hide: function () {
        this.slider_header_down.removeClass('headersliderF');
        this.slider_header_up.addClass('headersliderF');
        $('#oe_header').hide()
    },
    header_show: function() {
        this.slider_header_up.removeClass('headersliderF');
        this.slider_header_down.addClass('headersliderF');
        $('#oe_header').show()
    },
    on_loaded: function(data) {
        ...
        this.$element.delegate('a', 'click', function(e, id) {
            e.stopImmediatePropagation();
            return self.on_menu_click(e, id);
        });
        this.$element.delegate('a', 'dblclick', function(e, id) {
            e.stopImmediatePropagation();
            return self.on_menu_dbl_click(e, id);
        });
        this.$secondary_menu.delegate('a', 'click', function(e, id) {
            e.stopImmediatePropagation();
            return self.on_menu_click(e, id);
        });
        this.$secondary_menu.find('.oe_toggle_secondary_menu').click(this.on_toggle_fold);
        $("#sliderhdr").click(this.on_slider_header);
        $('.oe_topbar_name').click(this.on_toggle_options);
        $('body').bind('click',function() { $('.oe_dropdown_menu').removeClass('oe_opened'); });
    },

    on_toggle_options: function (event) {
        event.stopImmediatePropagation();
        $('.oe_dropdown_menu').toggleClass('oe_opened');
    },
    on_toggle_mobile_nav: function () {
         $('.menu-flexible').toggleClass('oe_show_menu');
    },
    ...
    on_slider_header: function() {
        this.sliderhdr ? this.header_show() : this.header_hide();
        this.sliderhdr=!this.sliderhdr;
        if (window.localStorage) {
            localStorage.setItem('header_menu_folded', this.sliderhdr.toString());
        }
    },
    /**
    ...
    on_menu_dbl_click: function(ev, id) {
        var $clicked_menu = $(ev.currentTarget),
            id = $clicked_menu.data('menu');
        if ($clicked_menu.parents('.oe_secondary_menu').is(':visible')==false || $clicked_menu.is('.leaf')) {
    ...
    do_show_secondary: function($sub_menu, $main_menu) {
        // TODO do that via live or delegate 
        var self = this;
        ...
        if (this.folded) {
            ...
            css.left = $main_menu.position().left;
...
openerp.web.WebClient = openerp.web.OldWidget.extend(/** @lends openerp.web.WebClient */{
    ...
    _get_version_label: function() {
        if (this.session.openerp_entreprise) {
            ...
            return _t("Enapps - Unsupported/Community Version");
        }
    },
    set_title: function(title) {
        title = _.str.clean(title);
        var sep = _.isEmpty(title) ? '' : ' - ';
        document.title = title + sep + 'Enapps';
    },
    start: function() {
        var self = this;
    ...
    show_application: function() {
        ...
        this.destroy_content();
        this.show_common();
        var $table = $(QWeb.render("Interface", {}));
    ...
    do_reload: function() {
        ...
        return this.session.session_reload().then(function () {
            openerp.connection.load_modules(true).then(
    ...
    on_hashchange: function(event) {
        ...
        var _compare_states = function(obj1, obj2) {
            var keys = ['id', 'model', 'view_type'];
            for (var k in keys) {
                if (!(obj1 && obj2 && obj1.hasOwnProperty(keys[k]) && obj2.hasOwnProperty(keys[k]) && obj1[keys[k]] === obj2[keys[k]])) {
                    return false;
                }
            }
            return true;
        };
        if (!_.isEqual(this._current_state, state) && !_compare_states(this._current_state, state)) {
