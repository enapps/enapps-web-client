...
openerp.web.ListView = openerp.web.View.extend( /** @lends openerp.web.ListView# */ {
    ...
    init: function(parent, dataset, view_id, options) {
        ...
        this.records.bind('change', function (event, record, key) {
            ...
            self.compute_aggregates((self.groups.get_selection().records).map(function(rec) { return {count: 1, values: rec} }));
        });
        this.no_leaf = false;
    },
    ...
    color_for: function (record) {
        ...
        for(var i=0, len=this.colors.length; i<len; ++i) {
            ...
            if (py.evaluate(expression, _.extend({bool: py.bool}, context))) {
                if (_.any(self.colors.filter(function(col){return col[2]===pair[2]}),function(el){return el[0]==='bold'}))
                    color_style+='font-weight: 700; ';
    ...
    on_loaded: function(data, grouped) {
        ...
        if (!!this.fields_view.fields.sequence) {this.options.reorderable=true;} else {this.options.reorderable=false;}
        ...
        this.$element.find('thead').delegate('th.oe-sortable[data-id]', 'click', function (e) {
            ...
            var $this = $(this);
            var move_refresh_content_bttn = (function(){
                var table_header = self.$element.find('thead'),
                    $button_reset_sort = self.$element.find('thead td.oe-actions button.refresh_content'),
                    position_left = self.$element.find("span.ui-icon-triangle-1-n, span.ui-icon-triangle-1-s").closest('th[data-id]').position().left,
                    $action_buttons = table_header.find('td.oe-actions button:not(.refresh_content):last'),
                    action_buttons_width = $action_buttons.length ? ($action_buttons.position().left + $action_buttons.outerWidth()) : 0,
                    pager_width = table_header.find('th.oe-list-pager').length ? table_header.find('th.oe-list-pager').outerWidth(): 0,
                    position_limit = Math.max(action_buttons_width, pager_width);
                if (position_left >= position_limit) {
                    $button_reset_sort.css({'position':'absolute','left': position_left+'px'});
                } else {
                    $button_reset_sort.css({'position':''});
                }
            });
            if (!!self.groups.datagroup.sort) self.groups.datagroup.sort.push($this.data('id'));
            ...
            if (!self.$element.find('thead td.oe-actions button.refresh_content').length) {
                var button_reset_sort = document.createElement('button'),
                    button_reset_sort_span = document.createElement('span');
                button_reset_sort.className = "oe_button refresh_content";
                button_reset_sort.type = "button";
                button_reset_sort_span.className = "icon-refresh";
                self.$element.find('thead td.oe-actions').append(button_reset_sort.appendChild(button_reset_sort_span).parentNode);
                self.$element.find('thead td.oe-actions button.refresh_content').click(function(){
                    self.groups.datagroup.sort = self.dataset._sort = [];
                    self.groups.view.reload_content();
                    self.$element.find("span.ui-icon-triangle-1-n, span.ui-icon-triangle-1-s").remove();
                    $(this).remove();
                });
            }
            move_refresh_content_bttn();
            self.reload_content();
        });
        this.$element.find('.oe-listview-select-all').click(function(event){
            self.$element.find('table > tbody.ui-widget-content > tr[data-id]').addClass('selected');
            self.do_select(self.get_selected_ids());
        });
    ...
    setup_columns: function (fields, grouped) {
        ...
        var self = this;
            self.fields_view.weights = _.reduce(self.fields_view.arch.children, function(memo,chld){if (chld.attrs.invisible!=='1') {return memo + (chld.attrs.width_weight ? parseFloat(chld.attrs.width_weight) : 1);} else {return memo + 0}},0)
        var noop = function () { return {}; };
        var field_to_column = function (field) {
            var name = field.attrs.name;
                var column = _.extend({id: name, tag: field.tag,
                width: (field.attrs.width_weight ? (field.attrs.width_weight/self.fields_view.weights)*100 : (1/self.fields_view.weights)*100)
                },
    ...
    reload_view: function (grouped, context, initial) {
        ...
        if (this.embedded_view) {
            return $.Deferred().done(callback).resolve(this.embedded_view);
        } else {
    ...
    reload_content: function () {
        var self = this;
        self.$element.find('.form-cont').remove();
        ...
        this.$element.find('.oe-listview-content').append(
            this.groups.render(function () {
                ...
                _.each(self.dataset.to_write,function(tw) {
                    _.each(tw.values,function(val,key){
                        self.records.get(tw.id).set(key,val);
                    })
                });
        ...
        this.$element.find('.oe-list-delete').attr('disabled', !self.get_selected_ids().lenght);
    ...
    do_delete: function (ids) {
        if (!(ids.length && confirm(_t("Do you really want to remove these records?")))) {
            var del_def = new $.Deferred();
            del_def.reject();
            return del_def.promise();
        }
        var self = this;
        var children_ids = [];
        _(ids).each(function (id) {
            if (!!self.records.get(id) && !!self.records.get(id).attributes.base_product && self.records.get(id).attributes.base_product===true)
                children_ids = children_ids.concat(self.get_children_ids(id));
        });
        ids = _.uniq(ids.concat(children_ids));
        return $.when(this.dataset.unlink(ids)).then(function () {
            var remove_records = (function(ids) {
                _(ids).each(function(id){
            ...
            self.compute_aggregates();
            // self.dataset.cache = [];
            // if (!_.isEmpty(children_ids)) {
            //     remove_records(children_ids);
            //     _.each(children_ids, function(item) {
            //         var index = self.dataset.ids.indexOf(item);
            //         self.dataset.ids.splice(index, 1);
            //     })
            // }
            self.reload_content();
        });
    },
    get_children_ids: function(id_dupl) {
        var self =this;
        return related_recs = _.map(_.filter(self.records.records, function(el) {
                if (el.attributes.parent_id && el.attributes.parent_id[0]===id_dupl) { return el }
            }), function(el) { return el.attributes.id });
    },
    /**
    ...
    do_select: function (ids, records) {
        ...
        if (!ids.length) {
            ...
            try {
                _(ids).each(function(id){
                    self.$element.find('tr[data-id="'+id+'"]').addClass('selected');
                });
    ...
    do_delete_selected: function () {
        var delete_def = this.do_delete(this.groups.get_selection().ids),
            self = this;
        delete_def.done(function(){
            self.configure_pager(self.dataset);
            self.compute_aggregates();
            self.reload_content();
        })
...
openerp.web.ListView.List = openerp.web.Class.extend( /** @lends openerp.web.ListView.List# */{
    ...
    init: function (group, opts) {
        ...
        this.$_element = $('<tbody class="ui-widget-content">')
            ...
            .delegate('tr[data-id]', 'click', function (e) {
                self.row_selected(e);
                if (self.options.editable!==null && ($(e.originalEvent.target).children('input[type="checkbox"]:not(:disabled)').length>0 || e.originalEvent.target.type=="checkbox")) 
                {
                    var row_id = self.row_id(e.currentTarget) || self.row_id($(e.currentTarget).parents('tr')) || $(e.currentTarget).parents('tr').attr('data-id');
                    var chk_values = {},
                        trg = e.originalEvent.target,
                        f_value = false;
                    if (trg.getElementsByTagName('input').length>0) {
                        f_name = e.originalEvent.target.getAttribute('data-field');
                        trg.getElementsByTagName('input')[0].checked=!(trg.getElementsByTagName('input')[0].checked);
                        f_value = trg.getElementsByTagName('input')[0].checked;
                    } else {
                        f_name = e.originalEvent.target.parentNode.getAttribute('data-field');
                        f_value = trg.checked;
                    }
                    chk_values[f_name]=f_value;
                    self.dataset.write(row_id,chk_values,{});
                    if (self.edition_form && self.edition_id===row_id && self.edition_form.$element.find('input[name="'+f_name+'"]')[0]) self.edition_form.$element.find('input[name="'+f_name+'"]')[0].click();
                }
            })
            .delegate('td.oe-record-delete button,td.oe-record-delete:has(button)', 'click', function (e) {
                e.stopPropagation();
            ...
            .delegate('td.oe-field-cell button', 'click', function (e) {
                ...
                $(self).trigger('action', [field.toString(), record_id, function () {
                    return self.dataset.parent_view && self.dataset.parent_view.reload() || self.reload_record(self.records.get(record_id));
                }]);
            })
            .delegate('th.oe-record-edit-link', 'click', function (e) {
                e.stopPropagation();
                $(e.target).find('button:visible').trigger('click');
            })
            .delegate('th.oe-record-edit-link button[name="edit"]', 'click', function (e) {
                self.row_clicked(e, 'form');
            })
            .delegate('th.oe-record-edit-link button.oe-edit-row-save', 'click', function (e) {
                e.stopPropagation();
            ...
            .delegate('tr[data-id]', 'dblclick', function (e) {
                self.row_clicked(e, 'form');
            })
            .delegate('tr[data-id]','contextmenu', function(e){
                e.preventDefault();
                self.context_menu_init(e);
                return false;
            });
    },
    ...
    get_selected_dataIds: function(e) {
        ...
        _.each($(e.target).closest('tbody').find('.selected'), function(el) {
            var selected_row_id = parseInt(el.getAttribute('data-id'))?parseInt(el.getAttribute('data-id')):(el.getAttribute('data-id'));
            selected_rows.push(selected_row_id);
        });
        if (selected_rows.length<=1) {
            var dataId = $(e.target).closest('tr').attr('data-id');
            selected_rows[0] = (parseInt(dataId)?parseInt(dataId):dataId);
        }
        return selected_rows;
    },
    context_menu_init: function(e) {
        var self = this;
            if ($('div[id="'+self.ctxt_menu_id+'"]').length>=2) {$('div[id="'+self.ctxt_menu_id+'"]').remove();}
            var $new_ctxt_menu = $('<div>', {id: self.ctxt_menu_id});
            $new_ctxt_menu.appendTo('body');
            var $cmenu = $("#"+self.ctxt_menu_id);
            $cmenu.find('ul').remove();
            $cmenu.append(QWeb.render("context_menu", {widget: self})).hide();
            var bindings = {};
            bindings[self.ctxt_menu_id + "_to_excel"] = function() {
                self.on_excel_export();
                return;
            };
            bindings[self.ctxt_menu_id + "_new_line"] = function() {
                var event_row_index = $(e.target).closest('tr[data-id]').index(),
                    row_index = event_row_index<0 ? self.$current.find('.selected').index()-1 : event_row_index;
                if (self.edition_form && self.edition_form.$element.parent().is('div')==false && self.edition_form.$element.index()-1<event_row_index)
                    --row_index;
                if (self.options.editable) self.view.do_add_record(row_index);
                return;
            };
            bindings[self.ctxt_menu_id + "_do_delete"] = function() {
                if (self.options.deletable){
                    var sel_rows = self.get_selected_dataIds(e);
                    self.view.do_delete(sel_rows);
                }
                return;
            };
            bindings[self.ctxt_menu_id + "_do_duplicate"] = function() {
                if (self.options.editable) {
                    ...
                    $.blockUI();
                    parent_form.do_save().done(function(){
                        sel_r = sel_rows;
                        // filtering selected records for being child records of already selected parent records
                        var selected_children=[];
                        _.each(sel_rows,function(id){selected_children=selected_children.concat(self.get_children_ids(id))});
                        sel_rows = _.difference(sel_rows,selected_children);
                        self.dataset.call('copy_list', [sel_rows, {}, self.dataset.context]).then(
                            function(res) {
                                self.dataset.buffer_rows_ids = [];
                                var i = 0;
                                _.each(res,function(res_id){
                                    self.records.add({id:res_id});
                                    self.dataset.ids.splice(row_index,0,res_id);
                                    self.reload_record(self.records.get(res_id));
                                    source_child[res_id]= sel_rows[i];
                                    i++;
                                });
                        },function(){$.unblockUI(); return;}).then(function(){
                            parent_form.do_save().then(
                                parent_form.reload().then(function(){
                                    _.each(source_child,function(key,val){
                                        var id_dupl = parseInt(val);
                                        if (!self.records.get(id_dupl))
                                            return;
                                        self.dataset.buffer_rows_ids.push(id_dupl);
                                        // if (self.records.get(id_dupl).attributes.base_product===true) {
                                        //     var related_recs = self.get_children_ids(id_dupl),
                                        //         ins_pos = self.dataset.buffer_rows_ids.indexOf(id_dupl)+1;
                                        //     self.dataset.buffer_rows_ids.splice(ins_pos,0,related_recs);
                                        //     self.dataset.buffer_rows_ids = _.flatten(self.dataset.buffer_rows_ids);
                                        // }
                                    });
                                }).then(function(){
                                    // searching for position of the last child of configurated product to append to
                                    var last_src_record = self.records.get(_.last(_.map(source_child,function(key,val){return key})));
                                    if (!!last_src_record.attributes.base_product && last_src_record.attributes.base_product === true) {
                                        var last_chld_id = _.max(_.map(self.get_children_ids(last_src_record.attributes.id),function(key,val){return key}),function(cid) {return self.records.get(cid).attributes.sequence});
                                        last_chld_pos = self.records.records.indexOf(self.records.get(last_chld_id));
                                    } else {
                                        // last_chld_pos = self.dataset.ids.indexOf(last_src_record.attributes.id); 
                                        last_chld_pos = self.records.records.indexOf(last_src_record);
                                    }
                                    // lines reordering
                                    if (self.view.page!=0) last_chld_pos-=1;
                                    self.move_lines(last_chld_pos,null);
                                    delete self.dataset.buffer_rows_ids;
                                    parent_form.do_save();
                                })
                            );
                        })
            ...
            bindings[self.ctxt_menu_id + "_do_cut"] = function() {
                if (self.options.reorderable){
                    var row_index = $(e.target).closest('tr').index();
                    var sel_rows = self.get_selected_dataIds(e);
                    self.dataset.buffer_rows_ids = sel_rows;
                    self.hide_lines(self.dataset.buffer_rows_ids);
                }
                return;
            };
            bindings[self.ctxt_menu_id + "_do_prepend"] = function() {
                if (self.options.reorderable && !!self.dataset.buffer_rows_ids && self.dataset.buffer_rows_ids.length){
                    $.blockUI();
                    self.move_lines($(e.target).closest('tr').index(),true);
                }
            };
            bindings[self.ctxt_menu_id + "_do_append"] = function() {
                if (self.options.reorderable && !!self.dataset.buffer_rows_ids && self.dataset.buffer_rows_ids.length){
                    $.blockUI();
                    self.move_lines($(e.target).closest('tr').index(),false);
                }
            };
            ...
            if (!!self.context_menu_items) {
                    ...
                    var new_item = $('<li>', {
                            ...
                            'class': 'conext_menu_list_item'
                        });
                    new_item.prepend('<span class="icon-script" style="padding-right: 5px;"></span>');
                        ...
                        if (self.dataset.parent_view && self.dataset.parent_view.is_dirty()) {
                            ...
                            d.then(function(){
                                ...
                                self.dataset.parent_view.do_save()
                                    .done(function() { form_saved_def.resolve(); return form_saved_def.promise(); })
                                    .fail(function() { $.unblockUI(); })
                            },function(){
                                form_saved_def.reject();
                                return form_saved_def.promise();
                            })
                        } else {
                            form_saved_def.resolve();
                ...
                _.each(self.context_menu_items.act_window, function(item) {
                    ...
                    new_item.prepend('<span class="icon-shortcut" style="padding-right: 5px;"></span>');
                    $cmenu.find('ul').append(new_item);
                    bindings[self.ctxt_menu_id+"act_window"+item.id] = function() {
                        var additional_context = {
                            active_id: self.get_selected_dataIds(e)[0],
                            active_ids: self.get_selected_dataIds(e),
                            active_model: self.view.model
                        };
                        self.view.rpc("/web/action/load", {
                            action_id: item.id,
                            context: additional_context
                        }, function(result) {
                            result.result.context = _.extend(result.result.context || {}, additional_context);
                            self.view.do_action(result.result, function() {self.view.reload()});
                        });
                    };
                });
            }
            $(e.target).contextMenu(self.ctxt_menu_id, {
                bindings: bindings,
                itemStyle: {"color": ""},
                onContextMenu: function(e) {
                    var close_ctxtmenu = function(){
                        clearTimeout(self.timer_to_close_ctxtmenu);
                        self.timer_to_close_ctxtmenu = setTimeout(function(){
                            $('#jqContextMenu').fadeOut(500);
                            $('#jqContextMenu').next().fadeOut(500);
                        },2000);
                    };
                    close_ctxtmenu();
                    $('#jqContextMenu').unbind('mouseout');
                    if ($(e.target).closest('tr[data-id]').hasClass('selected')==false) {
                        $(e.target).closest('tbody').find('.selected').removeClass('selected');
                        $(e.target).closest('tr[data-id]').addClass('selected');
                    };
                    if (!!self.options.editable) {
                        $('[id^="' + self.ctxt_menu_id + '_new_line"]').removeClass("oe-m2o-disabled-cm");
                    } else {
                        $('[id^="' + self.ctxt_menu_id + '_new_line"]').addClass("oe-m2o-disabled-cm");
                    }
                    if (!!self.options.deletable) {
                        $('[id^="' + self.ctxt_menu_id + '_do_delete"]').removeClass("oe-m2o-disabled-cm");
                    } else {
                        $('[id^="' + self.ctxt_menu_id + '_do_delete"]').addClass("oe-m2o-disabled-cm");
                    }
                    if (!!self.options.editable) {
                        $('[id^="' + self.ctxt_menu_id + '_do_duplicate"]').removeClass("oe-m2o-disabled-cm");
                    } else {
                        $('[id^="' + self.ctxt_menu_id + '_do_duplicate"]').addClass("oe-m2o-disabled-cm");
                    }
                    if (self.options.reorderable && self.records.length>1 && ((!!self.dataset.buffer_rows_ids && self.dataset.buffer_rows_ids.length==0) || (!self.dataset.buffer_rows_ids))) {
                        $('[id^="' + self.ctxt_menu_id + '_do_cut"]').removeClass("hidden");
                    } else {
                        $('[id^="' + self.ctxt_menu_id + '_do_cut"]').addClass("hidden");
                    }
                    if (self.options.reorderable && (!!self.dataset.buffer_rows_ids && self.dataset.buffer_rows_ids.length)) {
                            $('[id="' + self.ctxt_menu_id + '_do_prepend"]').removeClass("hidden");
                            $('[id="' + self.ctxt_menu_id + '_do_append"]').removeClass("hidden");
                    } else {
                        $('[id="' + self.ctxt_menu_id + '_do_prepend"]').addClass("hidden");
                        $('[id="' + self.ctxt_menu_id + '_do_append"]').addClass("hidden");
                    }
                    $('#jqContextMenu').mouseout(function(){close_ctxtmenu();});
                    $('#jqContextMenu').mouseover(function(){clearTimeout(self.timer_to_close_ctxtmenu);});
                    return true;
                    }, menuStyle: {width: "200px"}
                });
            $(e.target).trigger(e);
        },
    hide_lines: function(ids) {
        var self = this;
        for (var i=0; i<ids.length; i++) {
            self.$current.find('tr[data-id="'+ids[i]+'"]').hide();
        }
    },
    get_children_ids: function(id_dupl) {
        var self =this;
    ...
    move_lines: function(index_pos, do_prepend_lines) {
        var self = this,
            row_index = index_pos, i=0, new_seq, indexed_position, to_move,
            is_above = self.dataset.ids.indexOf(self.records.records[row_index].attributes.id)>self.dataset.ids.indexOf(self.dataset.buffer_rows_ids[0])?0:1,
            do_prepend = (do_prepend_lines===true || null)?1:0,
            buffer_ids = self.dataset.buffer_rows_ids;
        var dataset_ids_index = self.dataset.ids.indexOf(self.records.records[Math.max(index_pos-do_prepend,0)].attributes.id);
        dataset_ids_index += self.dataset.ids.indexOf(self.dataset.buffer_rows_ids[0])<dataset_ids_index ? (-self.dataset.buffer_rows_ids.length+1) : 1;
        if (do_prepend_lines!=null) {
            self.$current.find('tr.selected').removeClass('selected');
            var to_prepend_id = self.$current.find('tr[data-id]:nth('+(index_pos+1*!do_prepend)+')').data('id');
            self.dataset.to_reorder.push([to_prepend_id, buffer_ids,]);
        }
        indexed_position = self.dataset.ids.indexOf(self.records.records[row_index].attributes.id)>self.dataset.ids.indexOf(_.last(buffer_ids))?row_index:row_index+1;
        if (_.any(buffer_ids, function(id){return !self.records.get(id)}) && !is_above) indexed_position+=1;
        _.each(buffer_ids, function(id) {
            to_move = self.records.get(id);
            if (!to_move) {
                is_above = 1;
                to_move = self.dataset.cache.filter(function(rec){return rec.id === id})[0];
            }
            else { self.records.remove(self.records.get(id)); }
            self.records.add(to_move.values || to_move, {'at':indexed_position+(i*is_above)+do_prepend*(-1)});
            to_move = self.records.get(id);
            to_move.trigger('change', to_move);
            if (do_prepend_lines!=null) self.$current.find('tr[data-id="'+id+'"]').addClass('selected');
            i++;
        });
        self.dataset.ids=_.difference(self.dataset.ids, self.dataset.buffer_rows_ids);
        dataset_ids_index = Math.max(dataset_ids_index,0);
        Array.prototype.splice.apply(self.dataset.ids, [dataset_ids_index, 0].concat(self.dataset.buffer_rows_ids));
        if (do_prepend_lines!=null) delete self.dataset.buffer_rows_ids;
        self.refresh_zebra();
        $.unblockUI();
    },
    run_object_action: function(action_id, res_ids, context) {
        var self = this;
        return self.view.rpc('/web/listview/run_object_action', {
                action_id: action_id,
                context: context,
                res_ids: res_ids
            }, function(response) {
                return self.on_run_object_action_finished(response);
            });
    },
    on_run_object_action_finished: function(response) {
        var self = this;
    },
    row_selected: function (e) {
        var self = this;
        e.stopPropagation();
    ...
    row_clicked: function (e, view) {
        var self = this;
        e.stopPropagation();
        if (self.group.datagroup.level>0) { self.view.widget_parent.flags.keep_group = true; }
    ...
    render_cell: function (record, column) {
        ...
        if (column.type=='boolean')
            {
                column.readonly = ((this.dataset.widget_parent.readonly && this.dataset.widget_parent.is_readonly())
                    || !this.view.fields_view.arch.attrs.editable
                    || !!column.readonly) ? 1 : 0;
            }
    ...
    add_link_create_line: function(paddTop) {
        if (this.options.editable
            && (_.has(this.view.fields_view.arch.attrs,'add_line_link') && this.view.fields_view.arch.attrs.add_line_link==="1")) {
            var self = this;
            var last_data_row = self.$current.find('tr[data-id]:last'),
                last_row = this.$current.find('tr:last'),
                strcode = '<div class="ea_add_newrow"><span class="icon-plus"></span> <a class="ea-list-add-link">Add line</a></div>';
            if (self.$current.find('div.ea_add_newrow').length==0) {
                this.$current.find('tr:last').after(strcode);
                if (last_data_row.length<last_row.length) {
                    this.$current.find('div.ea_add_newrow').css('top',paddTop);
                }
                this.$current.find('a.ea-list-add-link').click(function() {
                    // this way to call create new record is better since it may trigger any listeners on this event for parent widget
                ...
                var pos_top = 0,
                    ...
                    if (line_edition_form.length!==0) {
                        ...
                        pos_top = self.$current.find('tr:first').position().top ;
                    }
                    $add_row_link.css('top',pos_top+'px');
            }
            var $addNewRow = self.$current.find('div.ea_add_newrow');
            if (last_data_row.index()==last_row.index() || last_data_row.index()>=3) { $addNewRow.css('position','static');}
            else {$addNewRow.css('position','absolute')}
    ...
    pad_table_to: function (count) {
        if (this.records.length >= count ||
            ...
            if (this.options.editable) {
                this.add_link_create_line();
                // this.$current.find('div.ea_add_newrow').css('position','static');
              }
            return;
        }
        var cells = [];
        ...
        if (this.options.editable) this.add_link_create_line();
    },
    /**
    ...
    refresh_zebra: function (from_index, offset) {
        ...
        var $elem = this.$current || this.view.$element.find('tbody.ui-widget-content:first');
        $elem.children(sel).each(function (i, e) {
    ...
    on_excel_export: function() {
        ...
        self.session.get_file({
            ...
            data: {data: JSON.stringify({
                ...
                headers : export_data.columns_names,
    ...
    get_export_data: function() {
        var self = this;
        var columns = self.view.visible_columns;
        ...
        rows = self.view.$element.find('.oe-listview-content > tbody > tr');
        export_rows = [];
            ...
            if($row.attr('data-id')){
                ...
                if ($row.hasClass("selected")){
                        $.each(export_columns_keys,function(){
                            ...
                            if (cell.classList.contains("oe_float")){
                                text = clear_text(text);
                                text = parseFloat(text);
                                export_row.push(text);
                            }
                            else if (cell.classList.contains("oe_integer")){
                                text = clear_text(text);
                                text = parseInt(text);
                                export_row.push(text);
                            }
                            else if (cell.classList.contains("oe_boolean")){
                                export_row.push($(cell).find('input:checked').length ? 'True' : 'False');
                            }
                            else{
        ...
        return {
            rows: export_rows,
            columns_names: export_columns_names
        }
    }
...
openerp.web.ListView.Groups = openerp.web.Class.extend( /** @lends openerp.web.ListView.Groups# */{
    ...
    render_groups: function (datagroups) {
        ...
        _(datagroups).each(function (group) {
            ...
            if (self.options.selectable) {
                // Not displayed since group column does not have that option
    ...
    render_dataset: function (dataset) {
        ...
        $.async_when().then(function() {
            dataset.read_slice(fields, options).then(function (records) {
            ...
            list.render();
            d.resolve(list);
            if (list.$current && list.$current.find('tr[data-id]:last').length>0) {
                var last_data_line = list.$current.find('tr[data-id]:last'),
                    paddTop = last_data_line.position().top + last_data_line.outerHeight();
                list.add_link_create_line(paddTop);
            }
            try {
                var parent_form = list.view.widget_parent.widget_parent.widget_parent.widget_parent;
                var parent_groups = parent_form.groups || false;
                if ( parent_groups && parent_groups.children[null] && !!parent_groups.children[null].edition_form ) {
                    list.view.widget_parent.widget_parent.widget_parent.widget_parent.groups.readjust_editionform_view();
                }
            }
            catch(err) {}
        });
    ...
    setup_resequence_rows: function (list, dataset) {
        ...
        list.$current.sortable({
            ...
            stop: function (event, ui) {
                var to_move_id = ui.item.data('id'),
                    prepend_id = ui.item.next().data('id'),
                    to_move = list.records.get(to_move_id),
                ...
                var record, index = to,
                    ...
                    list.dataset.to_reorder.push([prepend_id, [to_move_id],]);
                    list.dataset.on_change();
                to_move.trigger('change', to_move);
                list.refresh_zebra();
            }
        });
            ...
            function (groups) {
                ...
                self.render_dataset(dataset).then(function (list) {
                    ...
                    if (list.$_element) {
                        ...
                        delete list.$_element;
                    }
                });
            });
        return $element;
    },
    post_render_expandable: function(list,dataset) {
        var self = this;
        _.each(list.columns.filter(function(col){return col.expandable==="1"}),function(fcol){
            _.filter(list.$current.find('[data-field="'+fcol.id+'"]'),function(fld){
                if (!_.isEmpty($(fld).children().last()) && $(fld).children()[0] && $(fld).children().last()[0].scrollHeight>50) {
                    var folded_box_css = {'max-height':'50px','overflow':'hidden'};
                    $(fld).children().last().css(folded_box_css);
                    $(fld).append($('<button>',{
                        'class': 'textarea_expand fold',
                        }).click(function(event){
                            event.stopPropagation();
                            if (this.classList.contains("fold")) {
                                $(this.previousElementSibling).css({'max-height':'initial','overflow':'initial'});
                                this.classList.remove("fold");
                                this.classList.add('unfold');
                            } else {
                                $(this.previousElementSibling).css(folded_box_css);
