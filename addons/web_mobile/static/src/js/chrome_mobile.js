...
openerp.web_mobile.Shortcuts =  openerp.web.OldWidget.extend({
    ...
    start: function() {
        ...
        this.rpc('/web/session/sc_list',{} ,function(res){
            ...
            $.mobile.changePage("#oe_shortcuts", 'slideup', false, true);
        });
    },
    on_clicked: function(ev) {
        ...
        if(!$('[id^="oe_list_'+res_id+'"]').html()){
            ...
            $.mobile.changePage('#oe_list_'+res_id, 'slideup', false, true);
        }
        ev.preventDefault();
        ev.stopPropagation();
    }
...
openerp.web_mobile.Menu =  openerp.web.OldWidget.extend({
    ...
    on_menu_click: function(ev, id) {
        ...
        if(!$('[id^="oe_sec_menu_'+id+'"]').html()){
            ...
            $.mobile.changePage('#oe_sec_menu_'+id, 'slideup', false, true);
        }
    }
});
...
openerp.web_mobile.Secondary =  openerp.web.OldWidget.extend({
    ...
    start: function(ev, id) {
        ...
        $.mobile.changePage("#"+this.element_id, 'slideup', false, true);
    },
    on_menu_click: function(ev, id) {
        ...
        if (child_len > 0) {
            ...
            if(!$('[id^="oe_sec_menu_'+id+'"]').html()){
                ...
                $.mobile.changePage('#oe_sec_menu_'+id, {transition: "slideup", reverse: false, changeHash: false });
            }
        }else {
            if(!$('[id^="oe_list_'+id+'"]').html()){
                ...
                $.mobile.changePage('#oe_list_'+id, 'slideup', false, true);
            }
        }
    }
});
